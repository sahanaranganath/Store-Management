...
if(isset($_POST['submit'])){
		...
		foreach($_POST['quantity'] as $key => $val){
			...
			$availabilty2=mysqli_query($con,"select * from products where id=$key ");
			$row2=mysqli_fetch_array($availabilty2);
			$var= $row2['productName'];
			 //echo $val;
			 //echo $_POST['quantity'];
			 if($row2['quantity']< $val){
				echo "<script>  alert(' The product: {$var} is over the available stock. Please select within the available stock.');</script>";
			}
			else{
				if($val==0){
				...
				echo "<script>  alert(' The product: {$var} is updated');</script>";
			}
...
$availabilty=mysqli_query($con,"select * from products where id=$qty ");
$row1=mysqli_fetch_array($availabilty);

 //echo $row1['quantity'];
 if($row1['quantity']<1){
	mysqli_query($con,"update products set productAvailability='Out of Stock' where id=$qty ");
}
else{
		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart-qty item">Stock Avilable</th>
...
?>
					...
					<td><h4 class='cart-product-description'><a href="product-details.php?pid=<?php echo htmlentities($pd=$row['id']);?>" ><?php echo $row['quantity'];?> </td>
					<td class="cart-product-quantity">
						<div class="quant-input">
								...
								<?php $availabilty3=mysqli_query($con,"select * from products where id=$key ");
			$row3=mysqli_fetch_array($availabilty3); ?>
				             <input type="text" min="1" max="<?php echo $row3['quantity']; ?>" value="<?php echo $_SESSION['cart'][$row['id']]['quantity']; ?>" name="quantity[<?php echo $row['id']; ?>]">
...
<input type="radio" name="paymethod" value="COD" checked="checked"> COD
 ...
 <button type="submit" name="ordersubmit" class="btn btn-primary">Confirm order</button>
